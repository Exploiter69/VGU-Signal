...
def test_pdfplumber_crosscheck_disagreement_is_visible(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        pdf,
        "_extract_pdfplumber",
        lambda _: "A different longer extraction " * 10,
    )
...
def test_pdfplumber_is_fallback_when_primary_extraction_fails(monkeypatch) -> None:
    ...
    assert any("PyMuPDF extraction failed" in warning for warning in result.quality.warnings)
...
def test_sparse_pdf_uses_ocr_and_records_parser_kind(monkeypatch) -> None:
    ...
    assert result.quality.extraction_kind == ExtractionKind.OCR


def test_sparse_pdf_ocr_failure_preserves_sparse_text_and_warning(monkeypatch) -> None:
    body = make_pdf("x")
    monkeypatch.setattr(pdf, "_ocr", lambda _: "")
    result = pdf.extract_pdf(
        evidence_id="ev",
...
def test_html_metadata_links_and_script_removal_are_deterministic() -> None:
    ...
    assert result.metadata == (
        ("article:published_time", "2026-09-20T10:00:00+05:30"),
